  ...
  "The Controller" should {
    "have a default gametstate of GameState.PREGAME" in {
      val controller = Controller(Field(slotNum = 5,
        players = Map[Int,Player]((1, Player(id = 1).resetAndIncreaseMana()),
          (2, Player(id = 2)))))
    ...
    "place a card on field" in {
      ...
      controller.field.players(controller.field.activePlayerId).fieldbar.cardArea.row(2).isDefined should be(true)
    }
    "draw a card" in {
      val controller = Controller(Field(slotNum = 5,
        players = Map[Int,Player]((1, Player(id = 1).resetAndIncreaseMana()),
          (2, Player(id = 2)))))
      controller.drawCard()
      controller.field.players(controller.field.activePlayerId).hand.length should be(5)
    }
    "setting player names" in {
      ...
      controller.field.players(controller.field.activePlayerId).name should be("Jan")
    ...
    "attacking" in {
      ...
      controller.field.players(controller.field.activePlayerId).fieldbar.cardArea.row(3).isEmpty should be(true)
    }
    "switching player" in {
      val controller = Controller(Field(slotNum = 5,
        players = Map[Int,Player]((1, Player(id = 1).resetAndIncreaseMana()),
      ...
      controller.switchPlayer()
      controller.field.players(controller.field.activePlayerId).name should be("Sam")
    }
    ...
    "undo step / redo step" in {
      ...
      controller.drawCard()
      controller.undo
      controller.field.players(controller.field.activePlayerId).hand.length should be(4)
      controller.redo
      controller.field.players(controller.field.activePlayerId).hand.length should be(5)
    }
    ...
    "should return the Winner when one player has 0 hp" in {
      val controller = Controller(Field(slotNum = 5,
        players = Map[Int,Player]((1, Player(id = 1).resetAndIncreaseMana()),
          (2, Player(id = 2))), turns = 2))
      controller.placeCard(Move(2, 2))
      controller.directAttack(Move(fieldSlotActive = 2))
      controller.getWinner().get should be(controller.field.players(controller.field.activePlayerId).name)
    }
